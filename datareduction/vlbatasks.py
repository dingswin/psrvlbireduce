...
def singlesource_calib(uvdataset, modelimage, snver, refant, doamp, solmins,
                       ...
                       weightit=0,flagwheremodelbelow=-1.0,normalise=False, **kwargs):
    """
    Input parameters
    ----------------
    kwargs : dict
        1. modelshift : a list of 2 float (default : [0,0])
            [ra_shift, dec_shift] of the model (in arcsec).
    """
    try:
        modelshift = kwargs['modelshift']
    except KeyError:
        modelshift = [0,0]
    ...
    if modelimage == None:
        calib.smodel[1] = 1.0
        calib.smodel[2] = modelshift[0]
        calib.smodel[3] = modelshift[1]
        calib.smodel[4:] = [0]
...
#    calib.cmodel = 'IMAG'
    ...
    calib.snver = snver
    calib()
def ____singlesource_calib(uvdataset, modelimage, snver, refant, doamp, solmins,
                       dostokesi, soltype, calibsnr, averageifs, uvrange=[0,0],
