...
if runfromlevel <= runlevel and runtolevel >= runlevel and expcnonfig['autoflag']:
...
runlevel  = runlevel + 1
printTableAndRunlevel(runlevel, snversion, clversion, inbeamuvdatas[0])
## Do dual-phscal calibration if requested, only when phscal, in-beam calibrator and target are nearly co-linear ######################
if config['dualphscalcal']:
    print "Carrying out dual-phscal calibration..."
    dualphscalsnver1 = 101
    vlbatasks.deletetable(inbeamuvdatas[0], 'SN', dualphscalsnver1)
    inbeamsrc = config['primaryinbeam'].strip()
    sntablepath = tabledir + inbeamsrc + '.icalib.p1.sn'
    if not os.path.exists(sntablepaht):
        print "INBEAMSRC.icalib.p1.sn not found; aborting"
        sys.exit()
    vlbatasks.loadtable(inbeamuvdatas[0], sntablepath, dualphscalsnver1)
    vlbatasks.
